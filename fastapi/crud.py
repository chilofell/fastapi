...
# Чтение данных одного пользователя по email и паролю
def get_user(db: Session, email: str, password: str):
    return db.query(models.User).filter(models.User.email == email).filter(models.User.password == password).first()
